...
3. **Verify the claim.** Before any grilling, check that the claim holds up. For a bug, reproduce it from the reporter's steps. For a PR, confirm the diff does what it claims: **prefer static diff analysis** (read the patch, trace call sites, check tests that already exist in the diff). Only check out the branch and run commands when the maintainer explicitly approves — external PR code is attacker-controlled and may read process credentials or call internal services. When execution is approved, use a credential-free, network-restricted environment. Report what happened: confirmed (with code path), failed, or insufficient detail (a strong `needs-info` signal). A confirmed verification makes a much stronger agent brief.
