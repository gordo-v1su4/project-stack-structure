...
Run a stateful `/grilling` session whose only output is **workflow** specs. Use the grilling discipline (relentless, a round of questions at a time, a recommended answer attached to each) aimed at the vocabulary and goal below. Create and edit specs as the grilling resolves things. **Before deleting any `workflows/*.md` file, show the user what will be removed and wait for explicit confirmation**; preserve the previous content in git so it can be recovered.
