...
import { buildRoughCutSections } from "../songSectionReview";
...
type JoinTabProps = {
  ...
  onPlayWhole: () => void;
  onPlaySection: (sectionIds: string[]) => void;
...
export function JoinTab({ previewSegments, sectionLabels, existingFootage, activeClip, onActiveClip, onPlayWhole, onPlaySection,
  ...
  return <div className="space-y-4">
    <Surface>
      ...
      <nav aria-label="Song sections" className="mt-4 flex flex-wrap gap-2">
        {sections.map(section => <button key={section.id} type="button"
          aria-pressed={section.id === currentSection.id} onClick={() => onActiveClip(section.cuts[0]!.index)}
          className={`rounded-md border px-3 py-2 text-left text-sm ${section.id === currentSection.id ? "border-accent bg-accent-tint text-fg-0" : "border-line bg-ink-1 text-fg-2 hover:border-accent"}`}>
          <span className="block">{section.label}</span>
          <span className="mt-1 block text-xs text-fg-3">{time(section.start)}–{time(section.end)}</span>
        </button>)}
      </nav>
    </Surface>

    <div className="space-y-3" aria-label="Song sections and footage">
      {[currentSection].map(({ id, label, sectionIds, cuts }) => <Surface key={id}>
        <div className="mb-3 flex items-center justify-between gap-3">
          <div><h3 className="text-sm font-medium text-fg-1">{label}</h3>
            <p className="mt-1 text-xs text-fg-3">{time(cuts[0]!.segment.musicStart)}–{time(cuts.at(-1)!.segment.musicEnd)}</p></div>
          <Button size="sm" disabled={busy} onClick={() => onPlaySection(sectionIds)}>Play {label}</Button>
        </div>
    ...
    <Surface aria-label="Selected rough-cut placement">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" onClick={() => onPlaySection(currentSection.sectionIds)} disabled={busy}>Play this section</Button>
