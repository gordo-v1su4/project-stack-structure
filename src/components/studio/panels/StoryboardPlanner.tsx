...
import { buildFreshFramePrompt, buildSequenceGridPrompt, buildStoryboardSequences, canonicalStoryboardReferences, resolveSequenceGridDirection,
  bindStoryboardJobToSequence, storyboardJobMatchesSequence, IMAGE_MODELS, IMAGE_PRICE_GUIDE, identifyStoryboardJob, serializeStoryboardJob, type GenerationBilling, type StoryboardImageModel,
...
export function StoryboardPlanner({ projectId, segments, references, assets, onAsset, onInspect, sourceFrames, sectionLabels, sectionDirections, locked = false }: {
  ...
  function reviewSequence(asset: GeneratedStudioAsset) {
    ...
    if (chosen !== undefined) return sequences.find((sequence) => sequence.id === chosen && sequence.planSignature);
  ...
  function approveFreshFrame(asset: GeneratedStudioAsset) {
    ...
    if (!asset.storyboard || !sequence || blocked) return;
    ...
    <div className="space-y-4">
      {assets.filter((asset) => asset.storyboard && asset.status === "completed").map((asset) => <article key={asset.id} className="space-y-3 rounded border border-zinc-700 p-3">
        ...
        <label className="block text-xs">Review against current story placement
          <select aria-label={`Story placement for ${asset.title}`} className={field} value={reviewSequence(asset)?.id ?? ""} disabled={blocked || busy || !!batch}
            ...
            {sequences.filter((sequence) => sequence.planSignature).map((sequence) => <option key={sequence.id} value={sequence.id}>{sequence.label} · {fmt(sequence.songStart)}–{fmt(sequence.songEnd)} · {sequence.direction}</option>)}
          </select>
        </label>
        ...
        {asset.storyboard?.kind === "grid" ? <>
          ...
          <div className="space-y-2 text-xs"><p>Fresh standalone image · {asset.reviewStatus ?? "pending"}. No automatic entry into Join.</p>
            <div className="flex gap-2"><button className={button} disabled={blocked || busy || !!batch || !reviewSequence(asset)} onClick={() => approveFreshFrame(asset)}>Approve frame for current story</button><button className={button} onClick={() => onAsset({ ...asset, reviewStatus: "rejected" })}>Reject frame</button></div>
