...
export function srtTimeToSeconds(value: string) {
...
export function parseSRT(input: string): SrtChunk[] {
  const text = String(input || "").replace(/\r/g, "").trim();
  if (!text) return [];
  return text.split(/\n{2,}/).map((block, blockIndex) => {
    const lines = block.split("\n").filter(Boolean);
    const maybeIndex = /^\d+$/.test(lines[0]?.trim() || "") ? Number(lines.shift()) : blockIndex + 1;
    const timing = lines.shift();
    if (!timing?.includes("-->")) throw new Error(`Invalid SRT block ${maybeIndex}: missing timing`);
...
export function exportSRT(chunks: SrtChunk[]) {
  return (chunks || [])
    .map((chunk, i) => {
...
function normalizeSections(sections: SrtChunk[], duration: number) {
...
export function distributeLyricsAcrossSections(lyrics: string, sections: SrtChunk[] = [], duration = 60) {
