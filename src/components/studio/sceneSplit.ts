...
function joinAssetUrl(baseUrl: string, jobId: string | undefined, assetPath: string | undefined) {
  if (!jobId || !assetPath) return undefined;
...
export function normalizeSplitterManifest(
  ...
  segments.forEach((segment, arrayIndex) => {
    ...
    const clipPath = readString(record.clip_path);
    const thumbnailPath = readString(record.thumbnail_path);
    ...
    normalized.push({
      ...
      thumbnailUrl: joinAssetUrl(baseUrl, jobId, thumbnailPath),
      clipUrl: joinAssetUrl(baseUrl, jobId, clipPath),
      assetPath: clipPath,
