...
import { REFERENCE_LANGUAGE_RULE, VISIBLE_WRITING_RULE } from "@/components/studio/referenceLanguage";
...
For ordinary visible shots, omit constraints.intent. Put partnership, attraction, contest goals and survival meaning in purpose. For example, two named people dancing requires their names and actions:["dancing"], not intent:"They move together rhythmically" or intent:"Their connection grows". A fracturing floor requires physicalStates:["damaged"], not intent:"The world is collapsing". Include intent only if the user explicitly requires the shot itself to establish an otherwise unobservable condition. Never manufacture such a condition from the logline or purpose.
...
Return only the specified JSON object. Keep internal checks out of every prose field.`;
...
Captioned footage is the starting material for new stories: combine footage.captionClusters with the song and optional user brief to propose ways of putting those pieces together. When no premise or outcome is locked, Faithful offers a straightforward interpretation, Bold a different twist, and Wildcard may propose a substantially different or surreal world. Use footage.captionClusters as evidence of available visible material. The story may require unfilmed shots; preserve those as explicit requirements instead of rewriting the story to fit a convenient clip. Do not claim footage exists for unsupported actions.
